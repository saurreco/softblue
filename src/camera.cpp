...
void Camera::init(int w, int h, glm::vec3 pos, glm::vec3 look, glm::vec3 up, float fov, float nearPlane)
{
    setFrustum(w, h, fov, nearPlane);
    setView(pos, look, up);
}

void Camera::setFrustum(int w, int h, float fov, float nearPlane)
{
    this->screenWidth = w;
    this->screenHeight = h;
